const data = {
    usuario: { 
        email : "jeregomez@gmail.com",
        usuario: "jere_gomez",
        contraseña: "Jeremias123",
        nacimiento: 13/4/2003,
        dni : 45978253,
        foto : "/images/users/foto_jeremias.jpg"
    },
    productos: [
        {   fotoProducto: '/images/products/remeraCortaET.jpg',
            nombre: 'I will be right here',
            descripcion: 'Remera con frase de ET, color celeste ',
            fechaDeCarga: '16/02/23'
        },
        {   fotoProducto: '/images/products/remeraLargaET.jpg',
            nombre: 'ET Long Sleeve',
            descripcion:'Remera con frase de ET, manga larga, color azul ',
            fechaDeCarga: '23/02/23'
        },
        {   fotoProducto: '/images/products/pelucheETcamperita.jpg',
            nombre: 'Peluche ET con camperita',
            descripcion: 'Peluche de ET para decorar o jugar con el',
            fechaDeCarga: '21/01/23'
        },
        {   fotoProducto: '/images/products/pelucheETcorazon.jpg',
            nombre: 'Peluche ET con corazon',
            descripcion: 'Peluche del 40 aniversario de ET',
            fechaDeCarga: '24/01/23'
        },
        {   fotoProducto: '/images/products/pelucheETbata.jpg',
            nombre: 'Peluche ET en bata',
            descripcion: 'Peluche de ET con bata, super suave y tierno',
            fechaDeCarga: '6/01/23'
        },
        {   fotoProducto: '/images/products/pelucheETbebe.jpg',
            nombre: 'Peluche ET Bebe',
            descripcion: 'Peluche super suave y tierno de ET',
            fechaDeCarga: '17/03/23'
        },
        {   fotoProducto: '/images/products/shotsET.jpg',
            nombre: 'Set de vasos de shot de ET',
            descripcion: 'Vasos de shot del aniversario 40 de ET',
            fechaDeCarga: '9/03/23'
        },
        {   fotoProducto: '/images/products/pinET.jpg',
            nombre: 'Pin The extra-terrestrial ',
            descripcion: 'Pin coleccionable de la tienda del mejor extraterestre',
            fechaDeCarga: '4/04/23'
        },
        {   fotoProducto: '/images/products/imanILET.jpg',
            nombre: 'Magnet I love ET',
            descripcion: 'Iman para decorar tu heladera de ET',
            fechaDeCarga: '27/02/23'
        },
        {   fotoProducto: '/images/products/imanBeGoodET.jpg',
            nombre: 'Magnet Be Good',
            descripcion: 'Iman para decorar tu heladera de ET',
            fechaDeCarga: '13/01/23'
        },
        {   fotoProducto: '/images/products/billeteraET.jpg',
            nombre: 'ET Wallet',
            descripcion: 'Billetera de ET',
            fechaDeCarga: '10/03/23'
        },
        {   fotoProducto: '/images/products/tazaET.jpg',
            nombre: 'ET Mug',
            descripcion: 'Taza con doble diseño de ET',
            fechaDeCarga: '28/02/23'
        },
        {   fotoProducto: '/images/products/llaveroETcamperita.jpg',
            nombre: 'ET Key Ring',
            descripcion: 'Llavero de ET con la figura de ET en 3D, para decorar tus llaves de la casa, del auto y mas',
            fechaDeCarga: '5/03/23'
        },
    ],
    comentarios: [
        {
        nombreDeUsuario: "juan_neira",
        textoDeComentario: 'El producto es increible',
        imagenDePerfil: '/images/users/foto_jeremias.jpg',
        },
        {
        nombreDeUsuario: "bocapasion",
        textoDeComentario: 'Podria tener un buzo de boca?',
        imagenDePerfil: '/images/users/foto_jeremias.jpg',
        },
        {
        nombreDeUsuario: "sofimaggio",
        textoDeComentario: 'Me encanta ET',
        imagenDePerfil: '/images/users/foto_jeremias.jpg',
        },
        {
        nombreDeUsuario: "lolo_123",
        textoDeComentario: 'Tienen tazas de ET?',
        imagenDePerfil: '/images/users/foto_jeremias.jpg',
        },
        {
        nombreDeUsuario: "justivergara",
        textoDeComentario: 'Traje mi peluche a Usuahia',
        imagenDePerfil: '/images/users/foto_jeremias.jpg',
        },
    ]
}

module.exports = data 
